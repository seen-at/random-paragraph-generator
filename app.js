...
form.addEventListener('submit', function (event) {
  // by default, forms submit to a server when clicked; prevented by using the function
  event.preventDefault();

  // contents in input are strings, so converted to integer
  const value = parseInt(amount.value);

  const random = Math.floor(Math.random() * text.length);

  // for empty, negative and values > 9, display 1 paragraph at random
